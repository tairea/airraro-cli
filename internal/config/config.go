...
type Config struct {
	...
	AirraroTenantId string            `toml:"tenant_id"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://airraro-api-ase1.ezycommerce.sabre.com/api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("AIRRARO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "airraro-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AIRRARO_TENANT_ID"); v != "" {
		cfg.AirraroTenantId = v
		cfg.markEnvOverride("AirraroTenantId")
		cfg.AuthSource = "env:AIRRARO_TENANT_ID"
	}
	...
	if cfg.AuthSource == "" && cfg.AirraroTenantId != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("AIRRARO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.AirraroTenantId
	if token == "" {
		return ""
	}
	if c.AirraroTenantId == "" {
		return ""
	}
	replacements := map[string]string{
		"token":             token,
		"tenant_id":         c.AirraroTenantId,
		"AIRRARO_TENANT_ID": c.AirraroTenantId,
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AirraroTenantId = token
	delete(c.envOverrides, "AirraroTenantId")
	c.updateFileConfigField("AirraroTenantId")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AirraroTenantId"] {
			out.AirraroTenantId = c.fileConfig.AirraroTenantId
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AirraroTenantId":
		c.fileConfig.AirraroTenantId = c.AirraroTenantId
	}
}
