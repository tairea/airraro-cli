...
Resource scoping:
  ...
  airraro-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  airraro-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  airraro-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  airraro-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  airraro-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/airraro-pp-cli/data.db)")
