...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "airraro-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRRARO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRRARO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "airraro-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/airraro-pp-cli/feedback.jsonl.
When ` + "`AIRRARO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRRARO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airraro-pp-cli feedback list
  airraro-pp-cli feedback list --limit 5
  airraro-pp-cli feedback list --json`,
