# Air Rarotonga Printed CLI Agent Guide

This directory is a generated `airraro-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
airraro-pp-cli doctor --json
airraro-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
airraro-pp-cli which "<capability>" --json
airraro-pp-cli <command> --help
```
...
```bash
airraro-pp-cli <command> --help
airraro-pp-cli <command> --dry-run --agent
```
